...
# Ensure 'decision_tree_model.pkl' is available in the environment or adjust the path.
try:
...
try:
    ...
    # In a real application, you'd save the fitted encoders or the mapping
...
def welcome_page():
    ...
    based on your personal and financial details. Get valuable insights and suggestions
    to improve your chances of loan approval.</p>
    """, unsafe_allow_html=True)
    st.image("https://img.freepik.com/free-vector/financial-profit-concept-illustration_114360-2747.jpg", use_column_width=True) # Replace with a relevant image URL
    ...
    if st.button("Predict Loan Status"):
        try:
            ...
            if loan_status == "Approved":
                st.markdown(f'<div class="prediction-result approved">Loan Status: {loan_status} 🎉</div>', unsafe_allow_html=True)
                st.markdown("<p>Congratulations! Your loan is likely to be approved.</p>", unsafe_allow_html=True)
            else:
                st.markdown(f'<div class="prediction-result rejected">Loan Status: {loan_status} 😞</div>', unsafe_allow_html=True)
                st.markdown("<p>Your loan application is likely to be rejected. Here are some suggestions to improve your chances:</p>", unsafe_allow_html=True)
                ...
                if income_annum < loan_amount / (loan_term * 12) * 2: # Simple heuristic: income should be at least twice the monthly installment
                     st.write("- **Increase your Income:** Consider increasing your income or applying for a lower loan amount.")
                if residential_assets_value + commercial_assets_value + luxury_assets_value + bank_asset_value < loan_amount * 1.5: # Simple heuristic: assets should be more than loan amount
                    st.write("- **Increase your Assets:** Building up your assets can demonstrate financial stability.")
                st.write("- **Reduce Existing Debt:** Lowering your existing debt can improve your debt-to-income ratio.")
                st.write("- **Consider a Co-applicant:** Applying with a co-applicant with a good credit history and stable income can increase approval chances.")
                st.write("- **Review Loan Terms:** Explore different loan products and terms that might be a better fit for your financial situation.")
